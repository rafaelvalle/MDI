...
forests for classification tasks, largely because these models do not require imputing missing data nor encoding categorical variables. We experiment on two benchmark datasets with missing categorical data, comparing the three classifiers trained on either non-imputed or differently imputed data with different degrees of nonmissing at random (MNAR) perturbation. The results show that it may only be beneficial to impute missing data to when there is a very high level of missing data. Additionally, we find that in some cases MNAR perturbation improves prediction accuracy by acting as a regularization technique.\par
