...
We train three different classifiers on the preprocessed data: decision trees, random forests, and deep neural networks. The deep neural network consists of four layers, each of the two hidden layers having 1024 nodes, and employs the adaptive learning rate method Adadelta \citep{zeiler2012} for the update rule. We explore the following hyperparameter space via Bayesian optimization \citep{snoek2012}: Momentum schedule (0 to 1), dropout regularization (No, Yes) and learning rate: (0.000001 to 0.01). We obtain an ensemble of the ten candidate models that achieve the lowest training error during Bayesian optimization. We construct prediction confidence intervals using the variation in an ensemble of neural networks, each model fit on the training data and applied to the test data \citep{heskes1997}. 

We also use an ensemble method to construct prediction intervals for decision trees and random forests. An ensemble of decision trees is formed by varying the maximum depth of the tree (5 to 2,500), and  for random forests, we vary the number of trees for random forests (10 to 2,500) and decision rule for the number of features to consider when looking for the best split (the square root or logarithm of total number of features). 
